...
function make_connected_trigger(position, current_soundblocks, trigger_count, node_count, target_mode) {
    var trigger = levelNodeWithTrigger();
    trigger.levelNodeTrigger.position = position;
    trigger.levelNodeTrigger.scale = { x:1, y:1, z:1 };

    for (let i=current_soundblocks+node_count+1; i<trigger_count+current_soundblocks+node_count+1; i++) {
        ...
        target.triggerTargetAnimation.animationName = "idle"; // Default animation name for the triggers
...
async function generate(file, node_count) {
    ...
    for (let t=0; t<tracks.length;t++) {
        ...
        unique_pitches.forEach(hz => sound_blocks.push(get_basic_sound_block({ x:0, y:t, z:-1 }, hz * ((tracks[t].isDrums)? 2.5:1), tracks[t].volume, tracks[t].isDrums)));

        // Create triggers linked to each sound block
        for (let i=0; i<unique_pitches.length; i++) {
            triggers.push(get_sound_trigger_block(i, t, i + node_count + 2 + current_soundblocks));
        }
        ...
        wall_block.levelNodeStatic.position = { x: 0.55, y: t, z: (unique_pitches.length - 1)/2 };
        wall_block.levelNodeStatic.scale = { x: 1, y: 1, z: unique_pitches.length };
        wall_blocks.push(wall_block);
    }

	const targetAnimModes = load().COD.Level.TriggerTargetAnimation.Mode;

    var trigger_stop = make_connected_trigger({ x:0, y:0, z:-3 }, sound_blocks.length, triggers.length, node_count, targetAnimModes.STOP);
    var trigger_start = make_connected_trigger({ x:0, y:1, z:-3 }, sound_blocks.length, triggers.length, node_count, targetAnimModes.START);
    var trigger_reset = make_connected_trigger({ x:0, y:2, z:-3 }, sound_blocks.length, triggers.length, node_count, targetAnimModes.RESET);


    return [...sound_blocks, ...triggers, ...wall_blocks, trigger_stop, trigger_start, trigger_reset];
}
