...
LABEL LOOP
    ...
    SET R2 Str.Rot.X ; forward rotation adds speed

    LESS R7 R2 -5   ; R2 < -5
    GREATER R6 R2 5 ; R2 > 5
    OR R7 R7 R6     ; R2 < -5 || R2 > 5
    ...
    SET R3 Str.Rot.Z ; steering rotation

    LESS R7 R3 -5   ; R3 < -5
    GREATER R6 R3 5 ; R3 > 5
    OR R7 R7 R6     ; R3 < -5 || R3 > 5
    AND R7 R7 Hol.Act ; must also be holding the wheel
    IF R7 keep_steer
      SET R3 0      ; -5 to 5 becomes 0
    LABEL keep_steer

    MUL R3 R3 DeltaTime ; multiply by delta
    MUL R3 R3 0.1 ; turn speed constant

    ADD car.Rot.X car.Rot.X R3 ; turn the car
    ...
    MUL R4 R4 DeltaTime ; multiply by delta
    MUL R5 R5 DeltaTime
    ...
    MUL R2 R0 DeltaTime ; multiply by delta
    MUL R3 R1 DeltaTime
